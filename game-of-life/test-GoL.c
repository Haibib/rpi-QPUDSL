...
void test_GoL(void)
{
    ...
    printk("B[1][1]=%d B[1][2]=%d B[1][3]=%d B[128][128]=%d\n",
           buf[GOL_B_OFF + 1*COLS + 1],
           buf[GOL_B_OFF + 1*COLS + 2],
           buf[GOL_B_OFF + 1*COLS + 3],
           buf[GOL_B_OFF + 128*COLS + 128]);


    for (uint32_t r = 1; r < ROWS - 1; r++) {
        for (uint32_t c = 1; c < COLS - 1; c++) {
            uint32_t a0 = buf[GOL_A_OFF + (r-1)*COLS + (c-1)];  // A[0:-2, 0:-2]
            uint32_t a1 = buf[GOL_A_OFF + (r-1)*COLS + c    ];  // A[0:-2, 1:-1]
            uint32_t a2 = buf[GOL_A_OFF + (r-1)*COLS + (c+1)];  // A[0:-2, 2:0]
            uint32_t a3 = buf[GOL_A_OFF + r*COLS + (c-1)    ];  // A[:-2, 1:-1]
            uint32_t a4 = buf[GOL_A_OFF + r*COLS + (c+1)    ];  // A[:-2, 1:-1]
            uint32_t a5 = buf[GOL_A_OFF + (r+1)*COLS + (c-1)];  // A[:-2, 1:-1]
            uint32_t a6 = buf[GOL_A_OFF + (r+1)*COLS + c    ];  // A[:-2, 1:-1]
            uint32_t a7 = buf[GOL_A_OFF + (r+1)*COLS + (c+1)];  // A[:-2, 1:-1]
            uint32_t expected = a0 + a1 + a2 + a3 + a4 + a5 + a6 + a7;
            uint32_t got = buf[GOL_B_OFF + r*COLS + c];
            if (got != expected)
