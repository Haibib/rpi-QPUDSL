	...
	volatile struct addGPU **gpu)
{
	...
	ptr = (volatile struct addGPU *)(vc - GPU_BASE);
	if (ptr == NULL)
	{
		mem_free(handle);
		mem_unlock(handle);
...
void vec_add_init(volatile struct addGPU **gpu, uint32_t n, uint32_t num_qpus)
{
	...
	uint32_t vc = ptr->mail[0] - offsetof(struct addGPU, code);
	uint32_t gpu_A_base   = vc + offsetof(struct addGPU, A);
    uint32_t gpu_B_base   = vc + offsetof(struct addGPU, B);
    uint32_t gpu_C_base   = vc + offsetof(struct addGPU, C);
