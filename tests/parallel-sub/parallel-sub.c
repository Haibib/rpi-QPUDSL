...
void sub_gpu_prepare(
	volatile struct subGPU **gpu)
{
	uint32_t handle, vc;
	volatile struct subGPU *ptr;
	...
	handle = mem_alloc(sizeof(struct subGPU), 4096, GPU_MEM_FLG);
	if (!handle)
	{
		qpu_enable(0);
	...
	ptr = (volatile struct subGPU *)(vc - GPU_BASE);
	if (ptr == NULL)
	{
		mem_free(handle);
		mem_unlock(handle);
	...
	ptr->handle = handle;
	ptr->mail[0] = vc + offsetof(struct subGPU, code);
	ptr->mail[1] = vc + offsetof(struct subGPU, unif);
...
void vec_sub_init(volatile struct subGPU **gpu, uint32_t n, uint32_t num_qpus)
{
	...
	sub_gpu_prepare(gpu);
	volatile struct subGPU *ptr = *gpu;
	memcpy((void *)ptr->code, subshader, sizeof ptr->code);
	...
	uint32_t vc = ptr->mail[0] - offsetof(struct subGPU, code);
	uint32_t gpu_A_base   = vc + offsetof(struct subGPU, A);
    uint32_t gpu_B_base   = vc + offsetof(struct subGPU, B);
    uint32_t gpu_C_base   = vc + offsetof(struct subGPU, C);
