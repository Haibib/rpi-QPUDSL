...
void uart_init(void) {
    ...
    dev_barrier();
    // Enable Aux (p. 8)
    PUT32(AUX_ENB, GET32(AUX_ENB) | (1<<0));
    dev_barrier();

    // Disable transmitter, receiver, and interrupts
    ...
    // Set UART work in 8-bit mode
    PUT32(AUX_MU_LCR_REG, 0x3);
    PUT32(AUX_MU_MCR_REG, 0x0);

    // Set BAUD rate (115200 - 270), (230400 - 153), (460800 - 66) (576000 - 53)
    PUT32(AUX_MU_BAUD_REG, 66);
...
void uart_disable(void) {
    ...
    // Disable interrupts and clear FIFO
    PUT32(AUX_MU_IER_REG, 0);
...
int uart_has_data(void) {
    // TODO: implement this!
    return (GET32(AUX_MU_LSR_REG) & (1<<0)) ? 1 : 0;
}
