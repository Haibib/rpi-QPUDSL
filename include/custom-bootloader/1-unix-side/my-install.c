...
static void usage(const char *msg, ...) {
    ...
    output("\nusage: %s [--trace-all] [--trace-control] [--baud <rate>] [--addr <addr>] "
           "([device] | [--last] | [--first] [--device <device>]) "
           "<pi-program>\n", progname);
    output("    pi-program = has a '.bin' suffix\n");
    output("        <host-program> --device <tty> --baud <rate> --input initial_state.txt\n");
    output("        <host-program> reads data in from <input.txt> if appropriate\n");
...
int main(int argc, char *argv[]) { 
    ...
    double timeout_secs = 10.0;
    int fd = set_tty_to_8n1(tty, B115200, timeout_secs);
    if(fd < 0)
    ...
    if(baud_rate != B115200) {
        fd = set_tty_to_8n1(tty, baud_rate, timeout_secs);
        if(fd < 0)
            panic("could not re-configure tty to requested baud rate\n");
    }
    ...
    if(!exec_argv)
        ...
        printf("executing input program \n");
