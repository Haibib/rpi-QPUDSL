...
void sw_uart_put8(sw_uart_t *uart, uint8_t b) {
    ...
    uint32_t n = uart->cycle_per_bit,
             u = n,
             s = cycle_cnt_read();
    s = write_cyc_until(tx, 0, s, n);
    for (unsigned i = 0; i < 8; i++) {
        s = write_cyc_until(tx, (b >> i) & 0x1, s, n);
    }
    s = write_cyc_until(tx, 1, s, n);
}

// optional: do receive.
//      EASY BUG: if you are reading input, but you do not get here in 
//      time it will disappear.
int sw_uart_get8_timeout(sw_uart_t *uart, uint32_t timeout_cyc) {
    // right away (used to be return never).
    if (!wait_until_cyc(uart->rx, 0, timeout_cyc, cycle_cnt_read())) {
        return -1;
    }
    uint32_t sample_at = cycle_cnt_read() + uart->cycle_per_bit + (uart->cycle_per_bit / 2);
    int val = 0;

    for (unsigned i = 0; i < 8; i++) {
        while ((int32_t)(cycle_cnt_read() - sample_at) < 0);
        val |= (gpio_read(uart->rx) & 1) << i;
        sample_at += uart->cycle_per_bit;
    }
    return val;
}
